...
export function NovaTemplate({ resumeData, styleSettings }: { resumeData: ResumeData; styleSettings?: ResumeStyleSettings }) {
  ...
  return (
    <div style={{ ...vars, fontFamily: 'var(--font-body)' }}>
      <header style={{ borderRadius: '18px', overflow: 'hidden', marginBottom: '18px' }}>
        <div style={{ background: colors.accent, color: accentText, padding: '16px 18px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', alignItems: 'center' }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: '10px', fontWeight: 950, letterSpacing: '2.2px', textTransform: 'uppercase', opacity: 0.92 }}>Resume</div>
              <div style={{ marginTop: '6px', fontSize: '34px', fontWeight: 950, letterSpacing: '0.2px', lineHeight: 1.03 }}>{getFullName(resumeData)}</div>
              <div style={{ marginTop: '10px' }}>
                <ContactStack resumeData={resumeData} color={accentText} mutedColor={accentText} fontSize={'10.2px'} />
              </div>
            </div>

            <Photo resumeData={resumeData} size={70} radius={20} borderWidth={0} background={'rgba(255,255,255,0.18)'} textColor={accentText} />
          </div>
        </div>
